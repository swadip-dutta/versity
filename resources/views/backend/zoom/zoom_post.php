...
//      //  $topic = $this->input->post('meetingTitle');
//      $scheduledate = $this->input->post('start_date');
//      $date=date_create($scheduledate);
//      $js_Date= date_format($date, "Y/m/d H:i");
//      $dteStart = new DateTime($js_Date);
     ...
     try {
        ...
        'start_date' => $time,
        'meetingTopic' => $topic,
        'timezone' => '(GMT+1:00) Brussels, Belgium',
    ]);

         $insert ="INSERT INTO zooms(host_ids, host_email, topic, start_time, start_date, start_url, join_url, password, category_id) VALUES('$z->uuid', '$z->host_id', '$z->host_email', '$z->topic', '$start_time', '$start_date', '$z->start_url', '$z->join_url', '$z->password', '$category_id')";
         $query = mysqli_query($db, $insert);

         if ($query) {
             header('location:zoom_list.blade.php');
         } else {
             echo "Not Successful";
         }
         print_r($z);
...
<h1 style="background: #333; color: #ddd;">The Metting Is Created. Please Close The Tab. And Refresh Zoom List To See
    The Meeting On The List.</h1>